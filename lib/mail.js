...
const THIN = "1px solid #dfe2e7", GROUP = "1.5px solid #9aa0ab", SECT = "2px solid #9aa0ab", OUTER = "1.5px solid #9aa0ab", HEADBASE = "2px solid #8a909c";
...
function colGroups(rep) {
  return [
    { label: "현재", cols: [
      { key: "cur", title: dayLabel(rep.currentDate), cur: true },
...
function renderTable(rep, fontPx, sparkMap) {
  ...
  const hGroup = `${base};border:${THIN};border-bottom:${THIN};font-weight:700;font-size:${fh}px`;      // 그룹행: 얇은 밑줄
  const hCol = `${base};border:${THIN};border-bottom:${HEADBASE};font-weight:600;font-size:${fh}px`;     // 컬럼행: 진한 밑줄
  const hSpan = `${base};border:${THIN};border-bottom:${HEADBASE};font-weight:700;font-size:${fh}px`;     // rowspan 헤더
  const period = rep.strip && rep.strip.length ? `${md(rep.strip[0])}~${md(rep.currentDate)}` : "";

  let top = `<th rowspan="2" style="${hSpan};border-left:${OUTER}">분류</th><th rowspan="2" style="${hSpan};border-left:${GROUP}">지표</th>`;
  let sub = "";
  ...
  top += `<th rowspan="2" style="${hSpan};border-left:${GROUP}">추세<br><span style="font-weight:400;font-size:${Math.max(8, fh - 2)}px;color:#8a909c">${period}</span></th>`;

  let body = "";
  rep.sections.forEach((sec, si) => {
    sec.rows.forEach((row, ri) => {
      const subDiv = ri > 0 && row.label.startsWith("유럽") ? `border-top:${GROUP};` : ""; // 미국↔유럽 구분
      const sTop = ri === 0 ? `border-top:${SECT};` : subDiv;
      body += `<tr>`;
      if (ri === 0) body += `<td rowspan="${sec.rows.length}" style="padding:${pad};border:${THIN};border-left:${OUTER};border-top:${SECT};background:#f6f7f9;font-size:${fh}px;font-weight:700;text-align:center;vertical-align:middle;white-space:nowrap">${sec.label}</td>`;
...
export function buildPdfHtml(rep, issues, sparkMap) {
  return `<!doctype html><html lang="ko"><head><meta charset="utf-8">
    ...
    <div><h2>${TITLE(rep)} <span style="font-size:11px;font-weight:400;color:#6b7280">· 데이터 기준 ${genStamp()} (KST)</span></h2>${renderTable(rep, 10, sparkMap)}</div>
  ...
  for (const date of dates) {
    ...
    DAILY_COLS.forEach((c, i) => {
      ...
      if (c.derive) {
        ...
        if (a != null && b != null) {
          cell.value = { formula: `${colLetter(idToCol[c.derive[0]])}${r}-${colLetter(idToCol[c.derive[1]])}${r}`, result: Number((a - b).toFixed(4)) };
          cell.numFmt = c.fmt;
        }
