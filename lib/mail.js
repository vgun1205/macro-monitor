...
function buildIssuesHtml(items, dateLabel, weekText, global = []) {
  ...
  const row = (n, color, title) => `<tr><td style="padding:4px 0;border-bottom:1px solid #eef1f4">
          <div style="font-size:14px;font-weight:600;color:#1a1d23;line-height:1.4">(<span style="color:${color}">${n.source || "-"}</span>) <a href="${n.link}" style="color:#1a1d23;text-decoration:none">${title}</a>${dt(n)}</div>
        </td></tr>`;
  const rows = items.length
    ? items.map((n) => row(n, "147b8c", n.title)).join("")
  ...
  const gRows = (global || []).map((n) => `<tr><td style="padding:4px 0;border-bottom:1px solid #eef1f4">
          <div style="font-size:14px;font-weight:600;color:#1a1d23;line-height:1.4">(<span style="color:#b8742a">${n.source || "-"}</span>) <a href="${n.link}" style="color:#1a1d23;text-decoration:none">${n.title_ko || n.title}</a>${dt(n)}</div>
  ...
  return `<div style="font-family:${FONT};background:#eef1f4;padding:12px 6px">
    <table role="presentation" width="820" cellpadding="0" cellspacing="0" style="max-width:820px;margin:0 auto;background:#fff;border-radius:10px;overflow:hidden;border:1px solid #e3e7ec">
      ...
      <tr><td style="padding:14px 20px 4px">
        <div style="font-size:14.5px;font-weight:800;color:#147b8c;border-left:4px solid #147b8c;padding-left:8px;margin:0 0 4px">오늘의 주요 기사 (국내)</div>
...
export async function sendIssuesMail(toOverride, opts = {}) {
  ...
  // 증분 수집: 실제 정기발송은 직전 발송 이후 신규만. 테스트(toOverride)는 최신분 전체(창 미변경).
  const isTest = !!toOverride;
  const lastRun = isTest ? 0 : Number(await getConfig("issues_last_run")) || 0;
  const items = await fetchIssues(10, lastRun);
  ...
  try {
    ...
    if (lastRun) global = global.filter((n) => (n.ts || 0) > lastRun); // 증분
    await enrichGlobal(global);     // 영문 본문 추출(요약·워드용)
    await translateGlobal(global);  // 제목 번역 + 한 문장 요약
    await translateBodies(global);  // 본문 전문 한글 번역(워드용)
  ...
  if (!opts.noAttach) {
    try {
      ...
      attachments.push({ filename: `위험관리MI_${genDate}.docx`, content: await buildIssuesDocx({ dateLabel: genDate, weekText: wk, items, global }) });
