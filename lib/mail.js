...
function buildIssuesHtml(items, dateLabel, weekText, global = []) {
  ...
  const gRows = (global || []).map((n) => {
    const gsum = n.summary_ko || (n.snippet ? n.snippet.slice(0, 180) : "");
    return `<tr><td style="padding:11px 0;border-bottom:1px solid #eef1f4">
          ...
          ${gsum ? `<div style="font-size:12.5px;color:#6b7280;line-height:1.6;margin-top:3px">${gsum}</div>` : ""}
          <div style="font-size:11px;color:#aeb3bb;line-height:1.5;margin-top:2px">${n.title_ko ? n.title : ""}</div>
        </td></tr>`;
  }).join("");
...
// 거시지표 모니터링(거시 리포트) 수신자 — 오전 9시 1회: 김지훈·최재호·윤혁중·서동현
const RECIPIENTS = [
  { email: "jihoon777.kim@samsung.com", hours: [9] }, // 김지훈
...
// 보험사 위험관리 MI(이슈 메일) 수신자 — 오전 9시: 김지훈
