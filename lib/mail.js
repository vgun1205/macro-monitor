...
function buildIssuesHtml(items, dateLabel, weekText, global = [], periodText = "") {
  ...
  const row = (n, color, title) => `<tr><td style="padding:4px 0;border-bottom:1px solid #eef1f4">
          <div style="font-size:14px;font-weight:600;color:#1a1d23;line-height:1.4"><span style="color:#${color};font-weight:700">(${n.source || "-"})${kindTag(n.kind)}</span> <a href="${n.link}" style="color:#1a1d23;text-decoration:none">${title}</a>${dt(n)}</div>
        </td></tr>`;
  ...
  const gRows = (global || []).map((n) => `<tr><td style="padding:4px 0;border-bottom:1px solid #eef1f4">
          <div style="font-size:14px;font-weight:600;color:#1a1d23;line-height:1.4"><span style="color:#b8742a;font-weight:700">(${n.source || "-"})</span> <a href="${n.link}" style="color:#1a1d23;text-decoration:none">${n.title_ko || n.title}</a>${dt(n)}</div>
  ...
  return `<div style="font-family:${FONT};background:#eef1f4;padding:12px 6px">
    <table role="presentation" width="820" cellpadding="0" cellspacing="0" style="max-width:820px;margin:0 auto;background:#fff;border-radius:10px;overflow:hidden;border:1px solid #e3e7ec">
      ...
      <tr><td style="padding:14px 20px 4px">
        <div style="font-size:14.5px;font-weight:800;color:#147b8c;border-left:4px solid #147b8c;padding-left:8px;margin:0 0 4px">🇰🇷 국내 위험관리 MI</div>
...
export async function sendIssuesMail(toOverride, opts = {}) {
  ...
  if (!opts.noAttach) {
    ...
    try {
      ...
      const { getCachedBriefingHtml } = await import("./briefing.js");
      let briefingHtml = "";
      try { briefingHtml = await getCachedBriefingHtml(); } catch (e) { console.error("[issues] briefing 읽기 실패:", e.message); }
      const arows = await exportArticles({ months: 120 });
      attachments.push({ filename: `MI_아카이브_${genDate}.html`, content: buildArchiveHtml(arows, { generatedAt: genDate, briefingHtml }), contentType: "text/html; charset=utf-8" });
...
export async function sendBriefingMail(toOverride, opts = {}) {
  ...
  const br = await generateBriefing({ days: Number(opts.days) || 7 });
  if (!br.html) return `skip(no-briefing,count=${br.count})`;
  const genDate = new Date(Date.now() + 9 * 3600 * 1000).toISOString().slice(0, 10);
  const { exportArticles } = await import("./archive.js");
  const { buildArchiveHtml } = await import("./archivehtml.js");
  const rows = await exportArticles({ months: 120 });
  const ahtml = buildArchiveHtml(rows, { generatedAt: genDate, briefingHtml: br.html });
  const transport = nodemailer.createTransport({ service: "gmail", auth: { user, pass } });
  await transport.sendMail({
    from: `RM AI Native <${user}>`, to,
    ...
    html: `<div style="font-family:'Malgun Gothic',sans-serif;font-size:14px;color:#1a1d23;padding:12px;line-height:1.7">
      <p><b>주간 위험관리 AI 브리핑</b>입니다. 첨부 아카이브 파일 <b>상단</b>에 이번 주 핵심 이슈·유의사항이 정리돼 있습니다.</p>
      <p style="color:#6b7280;font-size:12.5px">· 기간 ${br.from} ~ ${br.to} · 기사 ${br.count}건 기반 · AI(Claude) 자동 생성(참고용)</p>
    </div>`,
    attachments: [{ filename: `MI_아카이브_브리핑_${genDate}.html`, content: ahtml, contentType: "text/html; charset=utf-8" }],
  });
  return `briefing-sent(${to.length},themes+watch,count=${br.count})`;
}
...
export async function sendArchiveMail(toOverride, opts = {}) {
  ...
  const html = buildArchiveHtml(rows, { generatedAt: genDate });
