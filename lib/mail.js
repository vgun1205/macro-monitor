...
import { fetchIssues } from "./news.js";
...
function renderNotes(rep, issues, autoIssues) {
  ...
  const news = (autoIssues && autoIssues.length)
    ? `<ul style="margin:6px 0 0;padding-left:18px;line-height:1.85">${autoIssues.map((n) => `<li style="margin-bottom:2px"><a href="${n.link}" style="color:#1F5FBF;text-decoration:none;font-weight:600">${n.title}</a> <span style="color:#8a909c;font-size:11.5px">— ${n.source}${n.date ? ` · ${n.date}` : ""}</span></li>`).join("")}</ul>`
    : `<div style="color:#8a909c;font-size:12px;margin-top:4px">(자동 수집 항목 없음)</div>`;
  return `<div style="font-size:13px;color:#1a1d23;line-height:1.85">
    ...
    <h3 style="font-size:16px;margin:6px 0">최근 이슈 · 규제 동향 <span style="font-size:12.5px;font-weight:600;color:#6b7280">(${weekLabel(rep.genDate || rep.currentDate)})</span></h3>
    ${news}
    ${manual}
    <div style="font-size:11px;color:#9aa0ab;margin-top:6px">※ Google News 자동 수집(키워드: 지급여력·K-ICS·보험 자본/규제) — 제목 클릭 시 원문. 별도 코멘트도 추가 가능.</div>
  </div>`;
}

export function buildHtml(rep, issues, sparkMap, autoIssues) {
  return `<div style="font-family:${FONT};color:#1a1d23;font-variant-numeric:tabular-nums;-webkit-font-smoothing:antialiased">
    ...
    ${linksBar()}
    <div style="margin-top:16px;border-top:${THIN};padding-top:12px">${renderNotes(rep, issues, autoIssues)}</div>
...
export function buildPdfHtml(rep, issues, sparkMap, autoIssues) {
  return `<!doctype html><html lang="ko"><head><meta charset="utf-8">
    ...
    </style></head><body>${buildHtml(rep, issues, sparkMap, autoIssues)}</body></html>`;
}
...
const RECIPIENTS = [
  { email: "jihoon777.kim@samsung.com", hours: [9, 13, 17] }, // 하루 3회
  { email: "jaeho24.choi@samsung.com", hours: [9] },          // 하루 1회(오전 9시)
  { email: "dh0407.seo@samsung.com", hours: [9] },            // 하루 1회(오전 9시)
  { email: "hj-act.yoon@samsung.com", hours: [9] },           // 하루 1회(오전 9시)
];
...
export async function sendReportMail(toOverride) {
  ...
  const autoIssues = await fetchIssues(6); // 규제·이슈 자동수집(헤드라인+링크)

  const html = buildHtml(rep, issues, sparkMap, autoIssues);
  ...
  try {
    ...
    attachments.push({ filename: `거시지표_${rep.currentDate}.pdf`, content: await buildPdf(buildPdfHtml(rep, issues, sparkMap, autoIssues)) });
    pdfOk = true;
  } catch (e) { pdfErr = e.message; console.error("[mail] PDF 생성 실패:", e.message); }
  // 최근 이슈·규제 동향 워드(.docx) 별도 첨부
  try {
    const { buildIssuesDocx } = await import("./issuedoc.js");
    const docxBuf = await buildIssuesDocx({ dateLabel: rep.genDate || rep.currentDate, weekText: weekLabel(rep.genDate || rep.currentDate), items: autoIssues });
    attachments.push({ filename: `규제동향_${rep.genDate || rep.currentDate}.docx`, content: docxBuf });
  } catch (e) { console.error("[mail] docx 생성 실패:", e.message); }
