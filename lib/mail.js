...
function buildIssuesHtml(items, dateLabel, weekText, global = [], periodText = "", briefing = null) {
  ...
  <div style="font-family:${FONT};background:#eef1f4;padding:12px 2%">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="width:100%;max-width:820px;margin:0 auto;background:#fff;border-radius:10px;overflow:hidden;border:1px solid #e3e7ec">
      ...
      <tr><td style="padding:12px 20px 0;text-align:center">
        <a href="${BASE}/archive/live" style="display:inline-block;background:#0f766e;color:#ffffff;text-decoration:none;font-size:14px;font-weight:700;padding:11px 20px;border-radius:9px">📱 모바일용 · 웹에서 아카이브 검색 열기 ▶</a>
      </td></tr>
      ${briefing && briefing.inner ? `<tr><td style="padding:12px 20px 2px">
        <details>
          <summary style="list-style:none;cursor:pointer;display:block;background:#0f766e;color:#ffffff;font-size:14px;font-weight:700;padding:11px 16px;border-radius:9px">🧭 주간 위험관리 AI 브리핑 · ${briefing.label || ""} <span style="font-size:9.5px;font-weight:400;color:#cdeee9">${briefing.range ? "(" + briefing.range + ")" : ""}</span></summary>
          <div style="padding:12px 2px 2px">${briefing.inner}</div>
        </details>
        <div style="border-top:1px dashed #d4d8de;margin:12px 0 0"></div>
      </td></tr>` : ""}
