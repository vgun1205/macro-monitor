...
const THIN = "1px solid #dfe2e7", GROUP = "1.5px solid #9aa0ab", SECT = "2px solid #8a909c", SUBDIV = "1.5px solid #b3b8c0", OUTER = "2px solid #6b7280", HEADBASE = "2px solid #8a909c";
...
function renderTable(rep, fontPx, sparkMap) {
  ...
  top += `<th rowspan="2" style="${hSpan}">추세<br><span style="font-weight:400;font-size:${sm}px;color:#8a909c">${period}</span></th>`;
  ...
  rep.sections.forEach((sec, si) => {
    sec.rows.forEach((row, ri) => {
      ...
      body += `<tr>`;
      if (ri === 0) body += `<td rowspan="${sec.rows.length}" style="padding:${pad};border:${THIN};border-top:${SECT};background:#f6f7f9;font-size:${fh}px;font-weight:700;text-align:center;vertical-align:middle;white-space:nowrap">${sec.label}<br><span style="font-weight:400;font-size:${sm}px;color:#7a818d">(단위:${sec.unit})</span></td>`;
      ...
      body += `<td style="padding:2px 6px;border:${THIN};${sTop}text-align:center;white-space:nowrap">${cell}</td>`;
      body += `</tr>`;
    });
  });
