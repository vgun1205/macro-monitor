...
function buildIssuesHtml(items, dateLabel, weekText) {
  const list = items.length
    ? items.map((n) => `<div style="margin:0 0 14px;padding-bottom:12px;border-bottom:1px solid #eceef1">
        <div style="font-size:15px;font-weight:700"><a href="${n.link}" style="color:#1a1d23;text-decoration:none">${n.title}</a></div>
        <div style="color:#8a909c;font-size:12px;margin:2px 0 4px">출처: ${n.source || "-"}${n.date ? ` · ${n.date}` : ""}</div>
        <div style="font-size:13px;color:#3a3f47;line-height:1.7">${n.snippet || ""}</div>
        <a href="${n.link}" style="color:#1F5FBF;font-size:12.5px;text-decoration:none">원문 보기 ▶</a>
      </div>`).join("")
    : `<div style="color:#8a909c">수집된 항목이 없습니다.</div>`;
  // 본문 글 폭을 1/3로 제한(가로로 길게 늘어지지 않게), 넘어가면 다음 줄로
  return `<div style="font-family:${FONT};color:#1a1d23;max-width:33%;min-width:240px;word-break:keep-all;overflow-wrap:break-word">
    <h2 style="margin:0 0 2px;font-size:20px;font-weight:800">보험사 위험관리 MI <span style="font-size:14px;font-weight:600;color:#6b7280">(${dateLabel})</span></h2>
    <div style="color:#6b7280;font-size:12.5px;margin:0 0 14px">경제 이슈 및 규제 동향 · ${weekText} · 키워드: 지급여력·K-ICS·자본/건전성·IFRS17·금리리스크</div>
    ${list}
    <p style="color:#9aa0ab;font-size:11.5px;margin-top:10px">※ 기사별 <b>원문 전문</b>은 첨부 워드(.docx) 참조. 출처: 네이버 뉴스 검색.</p>
  </div>`;
}
...
export async function sendIssuesMail(toOverride) {
  ...
  const to = toOverride ? toOverride.split(",").map((s) => s.trim()).filter(Boolean) : RECIPIENTS.map((r) => r.email);
  ...
  try {
    ...
    attachments.push({ filename: `보험사위험관리MI_${genDate}.docx`, content: await buildIssuesDocx({ dateLabel: genDate, weekText: wk, items }) });
  ...
  await transport.sendMail({
    ...
    html: buildIssuesHtml(items, genDate, wk),
    attachments,
  });
  return `sent(${to.length},news=${items.length})`;
}
