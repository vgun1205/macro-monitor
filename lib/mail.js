...
function buildIssuesHtml(items, dateLabel, weekText, global = [], periodText = "", briefingInner = "") {
  ...
  <div style="font-family:${FONT};background:#eef1f4;padding:12px 2%">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="width:100%;max-width:820px;margin:0 auto;background:#fff;border-radius:10px;overflow:hidden;border:1px solid #e3e7ec">
      ...
      ${briefingInner ? `<tr><td style="padding:14px 20px 2px">
        <div style="font-size:14.5px;font-weight:800;color:#0f766e;border-left:4px solid #0f766e;padding-left:8px;margin:0 0 6px">🧭 주간 위험관리 AI 브리핑</div>
        ${briefingInner}
...
export async function sendIssuesMail(toOverride, opts = {}) {
  ...
  await transport.sendMail({
    ...
    html: buildIssuesHtml(items, genDate, wk, global, periodText, (briefings[0] && briefings[0].inner) || ""),
    attachments,
  });
