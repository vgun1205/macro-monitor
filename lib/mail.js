...
function buildIssuesHtml(items, dateLabel, weekText) {
  const list = items.length
    ? items.map((n) => `<div style="margin:0 0 14px;padding-bottom:12px;border-bottom:1px solid #eceef1">
        ...
        <div style="color:#8a909c;font-size:12px;margin:2px 0 4px">${[n.source, n.date].filter(Boolean).join(" · ")}</div>
        <div style="font-size:13px;color:#3a3f47;line-height:1.7">${(n.snippet || "").slice(0, 220)}</div>
        <a href="${n.link}" style="color:#1F5FBF;font-size:12.5px;text-decoration:none">원문 보기 ▶</a>
      </div>`).join("")
    : `<div style="color:#8a909c">수집된 항목이 없습니다.</div>`;
  return `<div style="font-family:${FONT};color:#1a1d23">
    ...
    <div style="color:#6b7280;font-size:12.5px;margin:0 0 14px">경제 이슈 및 규제 동향 · ${weekText} · 키워드: 지급여력·K-ICS·보험 자본/규제</div>
    ${list}
...
export async function sendIssuesMail(toOverride) {
  ...
  const items = await fetchIssues(8);
  ...
  try {
    ...
    attachments.push({ filename: `경제이슈_규제동향_${genDate}.docx`, content: await buildIssuesDocx({ dateLabel: genDate, weekText: wk, items }) });
