...
function buildIssuesHtml(items, dateLabel, weekText, global = [], periodText = "", briefing = null) {
  ...
  // 상단 링크 버튼 2개(나란히·다른 색·한 줄) — 아카이브 검색(남색) / AI 브리핑(청록)
  const btnA = `<a href="${BASE}/archive/live" style="display:block;text-align:center;background:#1f3b57;color:#ffffff;text-decoration:none;font-size:13.5px;font-weight:700;padding:12px 6px;border-radius:10px;white-space:nowrap">📱 아카이브 검색 ▶</a>`;
  const wkShort = briefing && briefing.label ? briefing.label.replace("주차", "주") : "";
  const btnB = (briefing && briefing.inner) ? `<a href="${BASE}/briefing" style="display:block;text-align:center;background:#0f766e;color:#ffffff;text-decoration:none;font-size:13.5px;font-weight:700;padding:12px 6px;border-radius:10px;white-space:nowrap">🧭 AI 브리핑${wkShort ? " · " + wkShort : ""} ▶</a>` : "";
  const btnRow = btnB
    ? `<tr><td style="padding:12px 14px 2px"><table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td style="width:50%;padding:0 4px" valign="top">${btnA}</td><td style="width:50%;padding:0 4px" valign="top">${btnB}</td></tr></table></td></tr>`
    : `<tr><td style="padding:12px 20px 2px">${btnA}</td></tr>`;
