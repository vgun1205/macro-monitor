...
function buildIssuesHtml(items, dateLabel, weekText, global = []) {
  ...
  const rows = items.length
    ? items.map((n) => `<tr><td style="padding:9px 0;border-bottom:1px solid #eef1f4">
          <div style="font-size:14.5px;font-weight:600;color:#1a1d23;line-height:1.5">· <span style="color:#147b8c">${n.source || "-"}</span>_<a href="${n.link}" style="color:#1a1d23;text-decoration:none">${n.title}</a>${dt(n)}</div>
        </td></tr>`).join("")
    : `<tr><td style="color:#8a909c;padding:12px 0">수집된 항목이 없습니다.</td></tr>`;
  // 🌐 글로벌 위험관리 MI — 제목만(한글 번역 제목 + 영문 원제목)
  const gRows = (global || []).map((n) => `<tr><td style="padding:9px 0;border-bottom:1px solid #eef1f4">
          <div style="font-size:14.5px;font-weight:600;color:#1a1d23;line-height:1.5">· <span style="color:#b8742a">${n.source || "-"}</span>_<a href="${n.link}" style="color:#1a1d23;text-decoration:none">${n.title_ko || n.title}</a>${dt(n)}</div>
          ${n.title_ko ? `<div style="font-size:11px;color:#aeb3bb;line-height:1.4;margin-top:1px">${n.title}</div>` : ""}
        </td></tr>`).join("");
  ...
  return `<div style="font-family:${FONT};background:#eef1f4;padding:18px 12px">
    <table role="presentation" width="640" cellpadding="0" cellspacing="0" style="max-width:640px;margin:0 auto;background:#fff;border-radius:10px;overflow:hidden;border:1px solid #e3e7ec">
      <tr><td style="background:#147b8c;background:linear-gradient(120deg,#1aa0ad,#0e6582);padding:24px 28px">
        <div style="color:#ffffff;font-size:28px;font-weight:800;letter-spacing:-0.5px;margin:0">위험관리 MI</div>
