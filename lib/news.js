...
async function fetchFssText(url, title) {
  ...
  try {
    ...
    // 1) 첨부 PDF 우선 추출
    let pdfHref = "";
    $('a[href*="fileDown.do"]').each((_, a) => { const lbl = `${$(a).attr("title") || ""} ${$(a).text() || ""}`; if (/\.pdf/i.test(lbl) && !pdfHref) pdfHref = ($(a).attr("href") || "").replace(/&amp;/g, "&"); });
    if (pdfHref) {
      try {
        const buf = await downloadPdf(pdfHref.startsWith("http") ? pdfHref : base + pdfHref);
        if (buf) { const t2 = cleanGovPdf(await pdfText(buf), title); if (t2.length > 200) return t2; }
...
// 손해보험협회 보도자료 본문 — 게시판 본문이 비어 있어 첨부 PDF를 받아 텍스트 추출.
async function fetchKniaText(url, title) {
...
export async function enrichFullText(items) {
  await Promise.all(items.map(async (n) => {
    n.text = n.source === "금융감독원" ? await fetchFssText(n.link, n.title)
      : n.source === "손해보험협회" ? await fetchKniaText(n.link, n.title)
