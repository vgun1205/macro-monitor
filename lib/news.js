...
// 첨부 파일명 정리(메일 첨부용)
function pdfName(raw, fb) {
  let s = (raw || "").replace(/\s*다운로드\s*$/, "").replace(/[\\/:*?"<>|\n\r\t]/g, "").replace(/\s+/g, " ").trim();
  s = s.replace(/\.+\s*(pdf|pd|p)?\s*$/i, "").trim(); // 끝 확장자·잔여(.., .p 등) 제거
  return `${(s || fb).slice(0, 110)}.pdf`;
}
  ...
  try {
    ...
    if (pdfHref) {
      try {
        ...
        if (buf) {
          n._pdf = { filename: pdfName(pdfLbl, (title || "금감원자료").slice(0, 40)), content: buf };
...
async function fetchKniaText(n) {
  ...
  try {
    ...
    if (!buf) return "";
    n._pdf = { filename: pdfName(lbl, (title || "협회자료").slice(0, 40)), content: buf };
...
export async function enrichFullText(items) {
  await Promise.all(items.map(async (n) => {
    n.text = n.source === "금융감독원" ? await fetchFssText(n)
      : n.source === "손해보험협회" ? await fetchKniaText(n)
