...
// 구글뉴스 기사(암호화 링크)를 네이버 제목 검색으로 실제 언론사 링크로 치환 → 본문 추출 가능해짐
async function resolveGnews(n) {
  const id = process.env.NAVER_CLIENT_ID, secret = process.env.NAVER_CLIENT_SECRET;
  if (!id || !secret) return false;
  try {
    const url = `https://openapi.naver.com/v1/search/news.json?query=${encodeURIComponent(n.title.slice(0, 60))}&display=5&sort=sim`;
    ...
    for (const it of j.items || []) {
      if (norm(decode(it.title)) !== norm(n.title)) continue; // 제목 앞부분 일치 확인
      const link = it.originallink || it.link;
      if (!link || /news\.google/.test(link)) continue;
      const dm = domainOf(link);
      n.link = link; n.source = mediaName(dm); n.major = isMajor(dm); n.gnews = false;
      if (it.description) n.snippet = decode(it.description);
      const d = it.pubDate ? new Date(it.pubDate) : null;
      if (d && !isNaN(d)) n.ts = d.getTime();
      return true;
    }
  } catch {}
  return false;
...
export async function enrichFullText(items) {
  await Promise.all(items.map(async (n) => {
    if (n.gnews) await resolveGnews(n); // 구글뉴스 → 실제 언론사 링크 치환(성공 시 본문 추출 가능)
    n.text = n.source === "금융감독원" ? await fetchFssText(n)
      ...
      : n.source === "손해보험협회" ? ""
      : n.gnews ? "" // 구글뉴스 링크는 암호화 리디렉트라 본문 추출 불가(제목·링크만)
...
const INS_KEYS = /보험|지급여력|K-?ICS|킥스|건전성|책임준비금|계리|손해보험|생명보험|할인율|자본확충|후순위|신종자본|내부통제|전산장애|업무연속성|사업연속성|위험관리위원회|위탁|소비자보호|분쟁조정|불완전판매|판매수수료|리베이트|모집질서|\bGA\b|약관|보험금|손해율|재보험|IFRS|경과조치|지배구조|리스크|위험관리|정상화계획|부실정리|금융복합기업집단|스트레스\s?테스트|유동성|주담대|주택담보|신용대출/i;
// 뉴스 관련성 필터(무관 기사 제거) — BCM·제3자 리스크 포함
const NEWS_REL = /보험|생명|손보|지급여력|K-?ICS|킥스|건전성|책임준비금|계리|CSM|할인율|자본|후순위|금융당국|금융위|감독원|손해율|BCP|업무연속성|재해복구|전산장애|위탁|외주|제3자|공급망|사이버|주담대|주택담보|신용대출/i;
...
async function fetchNaver(id, secret, limit) {
  const results = await Promise.all(QUERIES.map(async (q) => {
    try {
      const url = `https://openapi.naver.com/v1/search/news.json?query=${encodeURIComponent(q)}&display=40&sort=date`; // 하루 기사 많은 날에도 24시간 창 커버(석간·심야 포함)
      const res = await fetch(url, { headers: { "X-Naver-Client-Id": id, "X-Naver-Client-Secret": secret } });
      const j = await res.json();
      return j.items || [];
...
// Google News RSS 보조망 — 네이버가 놓친 기사 캐치(제목·출처·링크만, 본문 추출 불가한 암호화 링크)
const GQUERIES = [
  "보험사 (지급여력 OR K-ICS OR 킥스 OR 경과조치)",
  "보험사 (자본확충 OR 신종자본증권 OR 후순위채)",
  "보험 (IFRS17 OR 계리가정 OR 할인율)",
  "보험사 (리스크관리 OR 내부통제 OR 위험관리위원회)",
  "금융 (업무연속성 OR 전산장애 OR 재해복구 OR 클라우드 장애)",
  "보험사 (주담대 OR 신용대출)",
];
async function fetchGoogleSupp(limit = 20) {
  const results = await Promise.all(GQUERIES.map(async (q) => {
    try {
      const ctrl = new AbortController();
      const t = setTimeout(() => ctrl.abort(), 8000);
  ...
  for (const xml of results) {
    for (const m of xml.matchAll(/<item>([\s\S]*?)<\/item>/g)) {
      ...
      const ts = d && !isNaN(d) ? d.getTime() : 0;
      out.push({ title, source: source || "구글뉴스", date: ts ? md(d) : "", link, snippet: "", ts, kind: "news", major: MAJOR_NAMES.has(source), gnews: true });
    }
  }
...
export async function fetchIssues(limit = 10, sinceTs = 0) {
  const id = process.env.NAVER_CLIENT_ID, secret = process.env.NAVER_CLIENT_SECRET;
  const [fss, fsc, kniaNews, kniaNotice, naver, gsupp] = await Promise.all([
    fetchFSS(6),
    fetchFSC(5),
    ...
    fetchGoogleSupp(20), // 구글 뉴스 보조망(네이버 누락분 캐치) — 같은 기사는 네이버판 우선
  ]);
  const news = [...naver, ...gsupp]; // 순서 중요: 중복 시 앞(네이버)이 남음
  ...
  if (sinceTs) {
    ...
    return merged.filter((n) => (n.kind === "news" ? (n.ts || 0) > sinceTs : kstDate(n.ts) >= fromDate)).slice(0, 40);
  }
