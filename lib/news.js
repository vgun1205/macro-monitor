...
// 손해보험협회 보도자료 본문 — 페이지엔 본문이 없고 첨부 PDF에만 있어, PDF를 받아 텍스트 추출.
function cleanKniaPdf(t) {
  let lines = (t || "").split("\n").map((s) => s.replace(/\s+/g, " ").trim());
  lines = lines.filter((s) => s
    && !/^-\s*\d+\s*-$/.test(s)                         // 페이지 번호 "- 1 -"
    && !/^보\s*도\s*자\s*료/.test(s) && !/^보도\s*시점/.test(s)
    && !/^(담당|문의|담당부서|담당팀|연락처|전화)/.test(s)
    && !/^\d{1,7}$/.test(s) && !/^[\d\s.\-():]{4,}$/.test(s));
  let txt = lines.join("\n");
  const ci = txt.search(/[※*]?\s*자세한\s*내용은\s*(첨부|붙임)/);
  if (ci > 30) txt = txt.slice(0, ci);
  return txt.replace(/\n{2,}/g, "\n").trim();
}
async function fetchKniaText(url) {
  ...
  try {
    ...
    const pres = await fetch(full, { headers: { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)" }, signal: ctrl2.signal });
    clearTimeout(t2);
    if (!pres.ok) return "";
    ...
    return cleanKniaPdf(r.text);
  } catch (e) { console.error("[news/knia-pdf] 실패:", e.message); return ""; }
}

// 진단: 서버(Vercel)에서 PDF 파싱 단계별 결과/에러 확인용
export async function debugPdf() {
  ...
  try {
    ...
    if (!href) return out;
    const buf = Buffer.from(await (await fetch(base + href, { headers: { "User-Agent": UA } })).arrayBuffer());
    out.pdfBytes = buf.length; out.head = buf.slice(0, 4).toString(); out.steps.push("download ok");
    const mod = await import("pdf-parse");
    out.pdfParseKeys = Object.keys(mod).slice(0, 8);
    const { PDFParse } = mod;
    const parser = new PDFParse({ data: new Uint8Array(buf) });
    const r = await parser.getText();
    out.textLen = (r.text || "").length; out.sample = (r.text || "").slice(0, 100); out.steps.push("parse ok");
...
export async function enrichFullText(items) {
  await Promise.all(items.map(async (n) => {
    n.text = n.source === "금융감독원" ? await fetchFssText(n.link, n.title)
      : n.source === "손해보험협회" ? await fetchKniaText(n.link)
