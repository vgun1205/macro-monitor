...
export async function enrichFullText(items) {
  await Promise.all(items.map(async (n) => {
    n.text = await fetchArticleText(n.link);
    const s = firstSentences(n.text);
    n.snippet = s && s.length >= 30 ? s : tidySnippet(n.snippet); // 요약을 완결 문장으로
  }));
  return items;
}
...
export async function fetchIssues(limit = 10) {
  ...
  const [fss, news] = await Promise.all([
    fetchFSS(5),
    (id && secret) ? fetchNaver(id, secret, limit) : fetchGoogle(limit),
  ]);
  const seen = new Set(), merged = [];
  for (const it of [...fss, ...news.filter((n) => NEWS_REL.test(n.title))]) {
