 ...
 *{box-sizing:border-box}
 ...
 .hero{background:#0e3a48;background-image:linear-gradient(135deg,#0a1a2e 0%,#103d4c 55%,#0f766e 130%);color:#fff;padding:38px 22px 26px}
 ...
 a{color:var(--acc)}
 .fss{color:#2563eb}.fsc{color:#6d28d9}.knia{color:#ea8207}.news{color:#0d9488}.global{color:#a16207}
 ...
 @media (max-width:640px){
  body{font-size:15px}
  .hero{padding:22px 15px 16px}
  h1{font-size:25px}
  h1:before{height:24px;width:10px}
  .sub{font-size:12.5px;margin:9px 0 12px}
  .chip{font-size:13px;padding:6px 11px}
  .wrap{padding:0 11px 70px}
  .box{padding:15px 15px}
  .bh,.sh{font-size:16.5px}
  .ti{font-size:16px}
  .mt{font-size:12.5px}
  .bd{font-size:14px;line-height:1.85}
  .ff,.fr{font-size:12.5px;padding:6px 10px}
  .fl{font-size:12px}
  .card{padding:13px 15px}
  #capBtn{right:12px;bottom:66px;height:42px;padding:0 14px;font-size:13px}
  #toTop{right:12px;bottom:12px;width:46px;height:46px;font-size:21px}
 }
</style></head><body>
 ...
 <div class="box" id="searchBox">
  ...
  <div class="filters">
   ...
   <div class="fg"><span class="fl">구분</span>
    ...
    <button class="fr" data-cat="fss">금감원</button>
    <button class="fr" data-cat="fsc">금융위</button>
    <button class="fr" data-cat="knia">손보협회</button>
...
<button id="capBtn" title="드래그로 영역을 선택해 이미지로 저장">📷 영역 캡처</button>
...
var CLS={fss_report:'fss',fsc_report:'fsc',knia_report:'knia',knia_notice:'knia',news:'news',global:'global'};
function esc(s){return (s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');}
function kdate(s){var p=(s||'').split('-');return p.length===3?(+p[0])+'년 '+(+p[1])+'월 '+(+p[2])+'일':(s||'');} // 한글 날짜
function kgen(s){var p=(s||'').split('-');return p.length===3?(+p[0])+'.'+(+p[1])+'.'+(+p[2]):(s||'');}
...
document.getElementById('sub').textContent='위험관리 MI 누적 기사 · 오프라인 검색 · 수집기간 '+(MIND?kdate(MIND):'-')+' ~ '+(MAXD?kdate(MAXD):'-')+' · 총 '+DATA.length+'건'+(${JSON.stringify(gen)}?' · 생성 '+kgen(${JSON.stringify(gen)}):'');
// 주간 브리핑 히스토리 버튼
(function(){
...
var F={q:'',alt:[],from:'',cat:'all'}, PAGE=1, PER=20;
function pass(a){
 if(F.cat==='news'&&a.k!=='news')return false;
 if(F.cat==='fss'&&a.k.indexOf('fss_')!==0)return false;
 if(F.cat==='fsc'&&a.k.indexOf('fsc_')!==0)return false;
 if(F.cat==='knia'&&a.k.indexOf('knia_')!==0)return false;
 if(F.cat==='global'&&a.r!=='global')return false;
 if(F.from&&a.d&&a.d<F.from)return false;
 if(F.alt&&F.alt.length){var hay=(a.t+' '+a.to+' '+a.s+' '+a.m+' '+a.p).toLowerCase();var ok=false;for(var t=0;t<F.alt.length;t++){if(F.alt[t]&&hay.indexOf(F.alt[t])>=0){ok=true;break;}}if(!ok)return false;}
 else if(F.q){var hy=(a.t+' '+a.to+' '+a.s+' '+a.m+' '+a.p).toLowerCase(); if(hy.indexOf(F.q)<0)return false;}
 return true;
}
function cardHtml(a){
 ...
 return '<div class="card" onclick="this.classList.toggle(\\'open\\')">'
  ...
  +'<div class="mt">'+esc(kdate(a.d))+' · '+esc(a.s)+(a.l?' · <a href="'+esc(a.l)+'" target="_blank" onclick="event.stopPropagation()">원문 ▶</a>':'')+(a.to?' · <span style="color:#94a3b8">'+esc(a.to)+'</span>':'')+'</div>'
...
document.getElementById('q').addEventListener('input',function(){F.q=this.value.trim().toLowerCase();F.alt=[];clearChips();reFilter();});
document.getElementById('home').onclick=function(){F={q:'',alt:[],from:'',cat:'all'};document.getElementById('q').value='';clearChips();resetFilterBtns();reFilter();window.scrollTo(0,0);};
var KEYWORDS=['지급여력','K-ICS','경과조치','자본확충','신종자본증권','IFRS17','할인율','내부통제','금감원 검사','BCM::BCM|BCP|업무연속성','사업연속성','위험관리위원회','전산장애','제3자','위탁','클라우드','재보험','주담대::주담대|주택담보','신용대출','삼성화재','삼성생명','캐노피우스::Canopius|캐노피우스','Solvency','ICS'];
(function(){var box=document.getElementById('kw');KEYWORDS.forEach(function(k){var parts=k.split('::'),label=parts[0],q=(parts[1]||parts[0]);var c=document.createElement('span');c.className='chip';c.textContent=label;c.onclick=function(){var on=c.classList.contains('on');clearChips();if(on){document.getElementById('q').value='';F.q='';F.alt=[];}else{c.classList.add('on');document.getElementById('q').value=label;F.q='';F.alt=q.toLowerCase().split('|');}reFilter();window.scrollTo(0,0);};box.appendChild(c);});})();
...
function doCapture(x,y,w,h){
 ...
 var opt={backgroundColor:'#eef1f5',scale:2,useCORS:true,windowWidth:document.documentElement.scrollWidth};
 if(w){opt.x=x;opt.y=y;opt.width=w;opt.height=h;}
...
capBtn.onclick=function(){
 ...
 var ov=document.createElement('div'); ov.style.cssText='position:fixed;inset:0;z-index:9998;cursor:crosshair;background:rgba(11,22,34,.12);touch-action:none';
 var box=document.createElement('div'); box.style.cssText='position:fixed;border:2px dashed #14b8a6;background:rgba(20,184,166,.12);display:none;z-index:9999;pointer-events:none';
 var hint=document.createElement('div'); hint.textContent='캡처할 영역을 드래그하세요 · 취소는 여기 탭 / Esc'; hint.style.cssText='position:fixed;top:12px;left:50%;transform:translateX(-50%);background:#0b1622;color:#fff;font-size:13px;font-weight:600;padding:9px 14px;border-radius:9px;z-index:10000;box-shadow:0 4px 14px rgba(0,0,0,.3);cursor:pointer';
 document.body.appendChild(ov); document.body.appendChild(box); document.body.appendChild(hint);
 var sx,sy,active=false;
 function cleanup(){ if(ov.parentNode)ov.remove(); if(box.parentNode)box.remove(); if(hint.parentNode)hint.remove(); document.removeEventListener('keydown',onKey); }
 function onKey(e){ if(e.key==='Escape')cleanup(); }
 document.addEventListener('keydown',onKey);
 hint.addEventListener('pointerdown',function(e){ e.stopPropagation(); cleanup(); });
 ov.addEventListener('pointerdown',function(e){ active=true; sx=e.clientX; sy=e.clientY; box.style.display='block'; box.style.left=sx+'px'; box.style.top=sy+'px'; box.style.width='0px'; box.style.height='0px'; try{ov.setPointerCapture(e.pointerId);}catch(_){} });
 ov.addEventListener('pointermove',function(e){ if(!active)return; var x=Math.min(sx,e.clientX),y=Math.min(sy,e.clientY),w=Math.abs(e.clientX-sx),h=Math.abs(e.clientY-sy); box.style.left=x+'px'; box.style.top=y+'px'; box.style.width=w+'px'; box.style.height=h+'px'; });
 ov.addEventListener('pointerup',function(e){ if(!active)return; active=false; var x=Math.min(sx,e.clientX),y=Math.min(sy,e.clientY),w=Math.abs(e.clientX-sx),h=Math.abs(e.clientY-sy); cleanup(); if(w<12||h<12)return; doCapture(x+window.pageXOffset,y+window.pageYOffset,w,h); });
};
render();
</script></body></html>`;
}
