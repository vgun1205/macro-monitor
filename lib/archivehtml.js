...
var KEYWORDS=['지급여력','K-ICS','경과조치','자본확충','신종자본증권','IFRS17','할인율','내부통제','금감원 검사','업무연속성','전산장애','제3자','위탁','클라우드','재보험','삼성화재','삼성생명','캐노피우스::Canopius','Solvency','ICS'];
(function(){var box=document.getElementById('kw');KEYWORDS.forEach(function(k){var parts=k.split('::'),label=parts[0],q=(parts[1]||parts[0]);var c=document.createElement('span');c.className='chip';c.textContent=label;c.onclick=function(){var on=c.classList.contains('on');clearChips();if(on){document.getElementById('q').value='';F.q='';}else{c.classList.add('on');document.getElementById('q').value=label;F.q=q.toLowerCase();}reFilter();window.scrollTo(0,0);};box.appendChild(c);});})();
