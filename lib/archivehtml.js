 ...
 *{box-sizing:border-box}
 ...
 a{color:var(--acc)}
 /* 통합 패널: 브리핑 + 기사검색을 하나의 카드로 */
 .panel{background:#fff;border:1px solid #e5e9f0;border-left:5px solid var(--acc);border-radius:16px;padding:20px 24px;margin:18px 0 6px;box-shadow:0 2px 12px rgba(11,22,34,.06)}
 .pdiv{border-top:1px solid #eef1f5;margin:16px 0}
 /* 주간 AI 브리핑 */
 .brief{margin:0}
 ...
 /* 기사 검색 섹션(같은 패널 내) */
 .sec{margin:0}
 .sh{font-size:19px;font-weight:800;cursor:pointer;user-select:none}
 .chev{float:right;color:var(--acc);transition:transform .2s;font-size:16px}
 .sec.collapsed .chev{transform:rotate(-90deg)}
 .sec.collapsed .sbody{display:none}
 .sbody .bar{background:#fff;top:0;padding-top:12px}
...
<div class="wrap">
 <div class="panel">
  ${meta.briefingHtml || ""}
  ${meta.briefingHtml ? '<div class="pdiv"></div>' : ""}
  <div class="sec" id="secSearch">
   <div class="sh" onclick="document.getElementById('secSearch').classList.toggle('collapsed')">🔎 기사 검색<span class="chev">▾</span></div>
   <div class="sbody">
    <div class="bar">
