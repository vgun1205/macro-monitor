...
var F={q:'',from:'',to:'',rg:''}, PAGE=1, PER=20;
function pass(a){
 if(F.rg&&a.r!==F.rg)return false;
 if(F.from&&a.d&&a.d<F.from)return false;
 if(F.to&&a.d&&a.d>F.to)return false;
...
function render(){
 ...
 document.getElementById('cnt').textContent=total+' / '+DATA.length+' 건';
...
document.getElementById('wOk').onclick=function(){
 ...
 F[curField]=y+'-'+pad2(m)+'-'+pad2(d); updateBtns(); ov.classList.remove('on'); reFilter();
};
document.getElementById('wClear').onclick=function(){F[curField]=''; updateBtns(); ov.classList.remove('on'); reFilter();};
...
document.getElementById('rg').addEventListener('change',function(){F.rg=this.value;reFilter();});
document.getElementById('home').onclick=function(){F={q:'',from:'',to:'',rg:''};document.getElementById('q').value='';document.getElementById('rg').value='';clearChips();updateBtns();reFilter();window.scrollTo(0,0);};
