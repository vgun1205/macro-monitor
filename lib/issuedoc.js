...
function entryParas(arr, isGlobal) {
  ...
  (arr.length ? arr : [null]).forEach((n) => {
    ...
    out.push(P([R(`- (${n.source || "-"}) ${cut(title, 54)}`, { size: 22, bold: true })], { spacing: { line: LINE, lineRule: "auto", before: 0, after: 30 } }));
    if (summ) out.push(P([R(cut(summ, 170), { size: 20, color: "444444" })], { spacing: { line: LINE, lineRule: "auto", before: 0, after: GAP } }));
  });
  return out;
}
...
export async function buildIssuesDocx({ dateLabel, weekText, items, global }) {
  ...
  // ── 2페이지: Summary 국내 ──
  ch.push(C([R("< Summary >", { bold: true, size: 30 })], { pageBreakBefore: true, spacing: { line: LINE, lineRule: "auto", before: 0, after: GAP } }));
  ch.push(P([R("1. 국내", { bold: true, size: 24 })], { spacing: { line: LINE, lineRule: "auto", before: 0, after: 80 } }));
  ch.push(box(entryParas(list, false)));
  // ── 3페이지: Summary 해외 ──
  ch.push(P([R("2. 해외", { bold: true, size: 24 })], { pageBreakBefore: true, spacing: { line: LINE, lineRule: "auto", before: 0, after: 80 } }));
