...
export async function buildIssuesDocx({ dateLabel, weekText, items }) {
  ...
  if (!items || !items.length) {
    ...
    items.forEach((n, i) => {
      ...
      const content = (n.text && n.text.length > 80) ? n.text : (n.snippet || "");
      if (content) children.push(new Paragraph({ spacing: { after: 60 }, children: [new TextRun({ text: content, size: 20 })] }));
