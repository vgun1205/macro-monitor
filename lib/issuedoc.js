...
const head = (t) => P([R(t, { bold: true, color: "1F5FBF" })], { spacing: { after: 30 } });
const linkP = (link, label) => P([new ExternalHyperlink({ link, children: [new TextRun({ text: label, size: SZ, font: FONT, characterSpacing: CSP, color: "1F5FBF", underline: {} })] })], { spacing: { before: 120 } });

// 본문 줄을 □/- 기호 + 들여쓰기로 변환한 문단 배열
function bodyParas(text, runOpt = {}) {
  return (text || "").split(/\n+/).map((s) => s.trim()).filter(Boolean).map((line) => {
    ...
    else if (/^[①-⑮]\s*/.test(line)) { lvl = 1; } // 번호 항목은 그대로, 들여쓰기만
    return P([R(out, runOpt)], { spacing: { line: LINE, lineRule: "auto", before: 0, after: 60 }, indent: lvl ? { left: 360 * lvl } : undefined });
  });
}
const para = (text) => P([R(text, { color: "333333" })], { spacing: { line: LINE, lineRule: "auto", after: 120 }, indent: { left: 360 } });
