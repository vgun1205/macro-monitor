...
const LINE = 300;   // 줄간격 1.25(과도한 줄간격 완화)
const GAP = 60;     // 문단 간 간격(단락 뒤, ≈3pt) — 과도한 여백 완화
const SZ = 22;      // 본문 11pt
...
function bodyParas(text, runOpt = {}) {
  ...
  return lines.map((line, idx) => {
    ...
    // 첫줄 위치(base) + 줄바꿈 정렬(콜론 뒤/마커 뒤 내어쓰기). 괄호 제목은 콜론정렬 제외, 내어쓰기 상한 1100.
    const base = kind === "box" ? 60 : kind === "dash" ? 480 : kind === "star" ? 760 : 0;
    const hw = bracket ? 0 : Math.min(hangW(out, isStar ? 20 : 22), 1100);
    return P([R(out, ro)], {
...
function entryParas(arr, isGlobal) {
  ...
  (arr.length ? arr : [null]).forEach((n) => {
    ...
    if (summ) out.push(P([R(cut(summ, 520), { size: 20, color: "444444" })], { indent: { left: tw }, spacing: { line: LINE, lineRule: "auto", before: 0, after: GAP } }));
  });
  return out;
}
...
export async function buildIssuesDocx({ dateLabel, weekText, items, global, periodText }) {
  const list = items || [], gl = global || [];
  const ch = [];

  // ── 1페이지: 표지(단독) ──
  ch.push(C([R("RM팀 위험관리 MI", { bold: true, size: 52 })], { spacing: { before: 1600, after: 120 } }));
  ch.push(C([R(`(${dateLabel})`, { size: 26 })], { spacing: { before: 0, after: periodText ? 60 : 140 } }));
  if (periodText) ch.push(C([R(periodText, { size: 18, color: "888888" })], { spacing: { before: 0, after: 140 } }));
