...
export async function buildIssuesDocx({ dateLabel, weekText, items, global }) {
  ...
  gl.forEach((n, i) => {
    ...
    // 번역문(한글) — 원문 위에 배치
    if (n.text_ko) {
      children.push(P([R("■ 번역문 (한글)", { bold: true, color: "1F5FBF" })], { spacing: { after: 30 } }));
      n.text_ko.split(/\n{1,}/).map((s) => s.trim()).filter(Boolean)
        .forEach((pa) => children.push(P([R(pa)], { spacing: { line: LINE, lineRule: "auto", before: 0, after: 60 } })));
    }
    children.push(P([R("■ 원문 (영문)", { bold: true, color: "1F5FBF" })], { spacing: { before: 60, after: 30 } }));
    const paras = (n.text || "").split(/\n{1,}/).map((s) => s.trim()).filter((s) => s.length > 0);
    if (paras.length) {
      paras.forEach((pa) => children.push(P([R(pa, { color: "555555" })], { spacing: { line: LINE, lineRule: "auto", before: 0, after: 60 } })));
    } else {
