...
export async function buildIssuesDocx({ dateLabel, weekText, items, global }) {
  ...
  // ── 🌐 글로벌 위험관리 MI (해외 자본·규제 동향) ──
  if (gl.length) {
    children.push(P([R(`🌐 글로벌 위험관리 MI (${gl.length}건)`, { bold: true, color: "B8742A" })], { pageBreakBefore: true, spacing: { after: 100, line: LINE, lineRule: "auto" } }));
    gl.forEach((n, i) => {
      children.push(P([R(`${i + 1}. ${n.title_ko || n.title}`, { bold: true })], { spacing: { before: 120, after: 20, line: LINE, lineRule: "auto" } }));
      if (n.title_ko) children.push(P([R(n.title, { color: "888888" })], { spacing: { after: 20 } }));
      children.push(P([R(`출처: ${n.source || "-"}${n.date ? `  ·  ${n.date}` : ""}`, { color: "888888" }), new TextRun({ text: "   원문 보기 ▶", size: SZ, font: FONT, characterSpacing: CSP, color: "1F5FBF" })], { spacing: { after: 40 } }));
      children.push(P([new ExternalHyperlink({ link: n.link, children: [new TextRun({ text: n.link, size: 18, font: FONT, characterSpacing: CSP, color: "1F5FBF", underline: {} })] })], { spacing: { after: 60 } }));
    });
  }
