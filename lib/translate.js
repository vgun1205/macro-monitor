...
// 글로벌 기사 본문(영문) 전문을 한국어로 번역 → text_ko. 기사별 병렬 호출.
export async function translateBodies(items) {
  const key = process.env.ANTHROPIC_API_KEY;
  if (!key || !items?.length) return items;
  const client = new Anthropic({ apiKey: key });
  await Promise.all(items.map(async (n) => {
    const body = (n.text || "").slice(0, 5000).trim();
    if (body.length < 100) return; // 본문 부실 시 건너뜀
    try {
      const msg = await client.messages.create({
        model: MODEL,
        max_tokens: 3500,
        system: "너는 보험·금융 전문 번역가다. 영문 기사 본문을 자연스러운 한국어로 번역한다. 보험·자본 전문용어는 정확히 옮기고, 문단 구분을 유지한다. 번역문만 출력하고 머리말·설명은 금지한다.",
        messages: [{ role: "user", content: body }],
      });
      const t = (msg.content || []).filter((b) => b.type === "text").map((b) => b.text).join("\n").trim();
      if (t) n.text_ko = t;
    } catch (e) { console.error("[translate-body] 실패:", e.message); }
  }));
  return items;
}
