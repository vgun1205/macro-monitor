...
export async function generateBriefing({ days = 7 } = {}) {
  ...
  const wHtml = watches.length
    ...
    : "";
  // 내부 콘텐츠만 반환(외곽 카드·헤더는 아카이브가 구성 → 주차별 전환용)
  const inner = `${tHtml}${wHtml}<div class="bf">* AI(Claude)가 아카이브 기사(${rows.length}건)를 근거로 자동 생성 · 참고용</div>`;
...
async function ensureBriefTable() {
  ...
  await pool().query(`CREATE TABLE IF NOT EXISTS weekly_briefings (
    week TEXT PRIMARY KEY, from_date TEXT, to_date TEXT, inner TEXT, count INT, created_at TIMESTAMPTZ DEFAULT now()
  );`);
...
export async function getRecentBriefings(n = 4) {
  ...
  const { rows } = await pool().query(
    `SELECT week, from_date, to_date, inner FROM weekly_briefings WHERE inner <> '' ORDER BY week DESC LIMIT $1`, [n]
  );
  return rows.map((r) => ({ week: r.week, from: r.from_date, to: r.to_date, inner: r.inner }));
}
...
export async function refreshWeeklyBriefing() {
  ...
  if (br.inner) {
    await pool().query(
      `INSERT INTO weekly_briefings (week, from_date, to_date, inner, count) VALUES ($1,$2,$3,$4,$5)
       ON CONFLICT (week) DO UPDATE SET from_date=EXCLUDED.from_date, to_date=EXCLUDED.to_date, inner=EXCLUDED.inner, count=EXCLUDED.count, created_at=now()`,
