...
export async function generateBriefing({ days = 7 } = {}) {
  ...
  if (!themes.length && !watches.length) return { html: "", from, to, count: rows.length, raw: text.slice(0, 600) };
  const tHtml = themes.map((x, i) =>
    `<div class="bt"><div class="btt"><b>${i + 1}. ${esc(x.t)}</b></div><div class="btb">${esc(x.b)}</div></div>`
  ).join("");
  const wHtml = watches.length
    ? `<div class="bw"><div class="bwh">⚠ 주목·유의</div>${watches.map((w) => `<div class="bwi">· ${esc(w)}</div>`).join("")}</div>`
    : "";
  const html = `<div class="brief collapsed" id="brief">
    <div class="bh" onclick="document.getElementById('brief').classList.toggle('collapsed')">🧭 주간 위험관리 AI 브리핑 <span class="bhd">(${from} ~ ${to} · 기사 ${rows.length}건 기반)</span><span class="bchev">▾</span></div>
    <div class="bbody">${tHtml}${wHtml}<div class="bf">* AI(Claude)가 아카이브 기사를 근거로 자동 생성 · 참고용</div></div>
  </div>`;
  return { html, from, to, count: rows.length };
}
...
// 주간 브리핑을 주 단위로 캐시. 같은 주는 저장본 재사용, 주가 바뀌면 새로 생성.
// 생성 실패 시 직전 주 저장본을 그대로 사용(빈 화면 방지).
export async function getWeeklyBriefing() {
  const kst = new Date(Date.now() + 9 * 3600 * 1000);
  const wk = isoWeekKey(kst);
  const savedWk = await getConfig("weekly_briefing_week");
  const savedHtml = await getConfig("weekly_briefing_html");
  if (savedWk === wk && savedHtml) return { html: savedHtml, week: wk, cached: true };
  const br = await generateBriefing({ days: 7 });
  if (br.html) {
    await setConfig("weekly_briefing_week", wk);
    await setConfig("weekly_briefing_html", br.html);
    return { html: br.html, week: wk, cached: false };
  }
  return { html: savedHtml || "", week: wk, cached: false }; // 생성 실패 → 직전본 유지
}
