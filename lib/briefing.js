...
// 발송 경로에서 호출(빠름): 저장된 주간 브리핑 HTML만 읽음. 생성 안 함.
export async function getCachedBriefingHtml() {
  return (await getConfig("weekly_briefing_html")) || "";
}
// 이번 주 브리핑이 아직 없으면 true(갱신 필요)
export async function isBriefingStale() {
  return (await getConfig("weekly_briefing_week")) !== curWeek();
}
// 무거운 생성(Claude) — 응답 후 after()에서 호출. 이번 주 브리핑을 만들어 저장.
export async function refreshWeeklyBriefing() {
  ...
  return { week: wk, count: br.count, ok: !!br.html };
}
