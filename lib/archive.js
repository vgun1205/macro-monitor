...
export async function saveArticles(items = [], global = []) {
  await ensureTable();
  const { firstSentences } = await import("./news.js"); // 풍부한 요약(4~5문장)
  const rows = [
    ...items.map((n) => ({
    ...
    ...global.map((n) => {
      // 글로벌 요약: 1문장 번역요약 대신 번역 본문에서 4~5문장 추출(풍부하게)
      const rich = firstSentences(n.text_ko || "", 560);
      return {
        pub_date: toDate(n.ts), region: "global", kind: "global", source: n.source || "-",
        title: n.title, title_ko: n.title_ko || null,
        summary: (rich && rich.length >= 60 ? rich : (n.summary_ko || n.snippet)) || null,
  ...
  let saved = 0;
  for (const r of rows) {
    // 중복 시 더 긴(풍부한) 요약·본문으로 갱신 → 가벼운 저장 후 나중에 본문이 채워짐
    const res = await p.query(
      `INSERT INTO mi_articles (pub_date, region, kind, source, title, title_ko, summary, body, body_ko, link)
       ...
       ON CONFLICT (source, title) DO UPDATE SET
         summary = CASE WHEN length(coalesce(EXCLUDED.summary,'')) > length(coalesce(mi_articles.summary,'')) THEN EXCLUDED.summary ELSE mi_articles.summary END,
         body    = CASE WHEN length(coalesce(EXCLUDED.body,''))    > length(coalesce(mi_articles.body,''))    THEN EXCLUDED.body    ELSE mi_articles.body END,
         body_ko = CASE WHEN length(coalesce(EXCLUDED.body_ko,'')) > length(coalesce(mi_articles.body_ko,'')) THEN EXCLUDED.body_ko ELSE mi_articles.body_ko END,
         title_ko = coalesce(mi_articles.title_ko, EXCLUDED.title_ko),
         pub_date = coalesce(mi_articles.pub_date, EXCLUDED.pub_date)`,
...
// 자체완결 HTML용 내보내기 — 최근 months개월치 전체(요약 + 본문 발췌)
export async function exportArticles({ months = 6, limit = 3000 } = {}) {
  await ensureTable();
  const { rows } = await pool().query(
    `SELECT to_char(pub_date,'YYYY-MM-DD') AS pub_date, region, kind, source, title, title_ko, summary,
            left(coalesce(body_ko, body, ''), 600) AS preview, link
     FROM mi_articles
     WHERE pub_date >= (CURRENT_DATE - ($1 || ' months')::interval)
...
// 아카이브 수집기간(최소~최대 발행일) — YYYY-MM-DD 문자열
export async function getArchivePeriod() {
  await ensureTable();
  const { rows } = await pool().query(
    "SELECT to_char(min(pub_date),'YYYY-MM-DD') AS f, to_char(max(pub_date),'YYYY-MM-DD') AS t FROM mi_articles WHERE pub_date IS NOT NULL"
  );
  return { from: rows[0]?.f || "", to: rows[0]?.t || "" };
}
