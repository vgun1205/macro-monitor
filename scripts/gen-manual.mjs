...
const H = (n, t) => P([R(`${n}. ${spread(t)}`, { bold: true, size: 24 })], { spacing: { line: LINE, lineRule: "auto", before: 320, after: 130 } });
// □ 문단: 앞 간격 200(≈10pt)으로 블록 구분을 명확히
const B = (t) => P([R("□ " + t, { bold: true })], { indent: { left: 360, hanging: 300 }, spacing: { line: LINE, lineRule: "auto", before: 200, after: 50 } });
const D = (t) => P([R("- " + t)], { indent: { left: 760, hanging: 280 }, spacing: { line: LINE, lineRule: "auto", before: 0, after: 40 } });
const S = (t) => P([R("* " + t, { size: 20, color: BLUE })], { indent: { left: 980, hanging: 220 }, spacing: { line: 240, lineRule: "auto", before: 0, after: 40 } });
...
ch.push(tbl(["구분", "기존 방식", "위험관리 MI"], [
  ["정보 수집", "금감원·금융위·협회·언론사를 각각 방문해 검색 (30분 이상)", "메일 1통으로 자동 도착 (3분 열람)"],
  ["해외 동향", "영문 기사 직접 검색·해석 부담", "한글 번역 제목·요약·본문 제공"],
  ["원문 확인", "사이트에서 다시 찾아 다운로드", "금감원·금융위·협회 원본 PDF 첨부"],
  ["주간 정리", "직접 요약·보고 자료 작성", "AI가 핵심 이슈 3~5개 자동 브리핑"],
  ["과거 사례", "예전 기사 다시 찾기 어려움", "아카이브에서 키워드·기간·기관별 즉시 검색"],
  ["누락 위험", "바쁘면 놓침", "매일 자동, 증분 방식으로 빠짐없이"],
], [1500, 3900, 4100]));
ch.push(B("핵심 강점 요약"));
