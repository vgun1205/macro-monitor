...
function hangFor(marker, text, defHang) {
  ...
  if (ci > 1 && ci <= 20) return Math.min(textW(full.slice(0, ci + 1) + " "), 1400);
  return defHang;
}
...
// N: 대시(-) 없는 하위 항목(①·Q. 등 자체 마커) — 위치는 - 와 동일
const N = (t) => { const hw = hangFor("", t, 330); return P([R(t)], { indent: { left: 480 + hw, hanging: hw }, spacing: { line: LINE, lineRule: "auto", before: 0, after: 40 } }); };
...
ch.push(N("① 제목 확인 : '위험관리 MI ｜ Daily & Archive (날짜)' 메일을 엽니다."));
ch.push(N("② 주간 흐름 파악(30초) : 상단 초록 버튼 '주간 위험관리 AI 브리핑'을 누르면 이번 주 핵심 이슈 3~5개와 유의사항이 바로 아래에 펼쳐집니다."));
ch.push(N("③ 오늘 기사 훑기(1분) : 국내 → 글로벌 순으로 헤드라인을 훑습니다. 제목 오른쪽 #태그(#K-ICS #자본 등)로 관심 기사만 골라 봐도 됩니다."));
ch.push(N("④ 자세히 보기(1분) : 관심 기사는 제목 클릭(원문 이동) 또는 첨부 워드(요약+본문 정리)로 확인합니다. 감독당국 자료는 첨부 원본 PDF를 바로 열면 됩니다."));
ch.push(N("⑤ 과거가 궁금하면 : '비슷한 일이 전에도 있었나?' → 첨부 아카이브(HTML)를 열어 키워드로 검색합니다. (6장 참고)"));
...
ch.push(B("FAQ"));
ch.push(N("Q. 모바일에서 첨부 HTML이 흐린 이미지로 보여요. → A. 메일앱의 미리보기일 뿐입니다. '모바일용 아카이브 검색' 버튼(웹)으로 여시면 됩니다."));
ch.push(N("Q. 어떤 기사는 요약이 짧아요. → A. 해당 언론사가 본문 수집을 막은 경우입니다. '원문 ▶' 링크로 확인해 주세요."));
ch.push(N("Q. 옛날 이슈를 찾고 싶어요. → A. 첨부 아카이브에서 키워드·기간·기관으로 검색하세요. 자료는 매일 자동 축적됩니다."));
ch.push(N("Q. 우리 회사(삼성화재)나 특정 해외사 소식만 모아볼 수 있나요? → A. 아카이브 키워드 칩의 '삼성화재', '캐노피우스'처럼 등록해 드립니다. 담당자에게 요청하세요."));
...
dx = dx.replace(/<w:spacing (?!w:val=")/g, '<w:wordWrap w:val="1"/><w:autoSpaceDE w:val="0"/><w:autoSpaceDN w:val="0"/><w:spacing ');
